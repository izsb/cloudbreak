    ...
    public ImageCatalog getImageCatalog(String catalogUrl)  {
        try {
            if (Objects.nonNull(catalogUrl)) {
                ...
                throw new ImageCatalogException(String.format("Failed to read an image catalog from '%s' content.", content));
            }
            throw new ImageCatalogException("Unable to fetch image catalog by the catalogUrl is null.");
