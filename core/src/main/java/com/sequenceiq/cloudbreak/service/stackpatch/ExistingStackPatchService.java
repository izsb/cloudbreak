...
public abstract class ExistingStackPatchService {
    ...
    public void apply(Stack stack) throws ExistingStackPatchApplyException {
        Optional<FlowLog> lastFlowLog = flowLogService.getLastFlowLog(stack.getId());
        if (lastFlowLog.isEmpty() || lastFlowLog.get().getStateStatus().equals(StateStatus.SUCCESSFUL)) {
            try {
        ...
        } else if (lastFlowLog.get().getStateStatus().equals(StateStatus.PENDING)) {
            ...
            String message = String.format("Last flow was status was %s for stack %s, skipping patch apply to preserve possible flow retry",
                    lastFlowLog.get().getStateStatus(), stack.getResourceCrn());
            throw new ExistingStackPatchApplyException(message);
        }
    }
