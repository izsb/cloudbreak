...
public class DeleteCloudFormationHandler implements CloudPlatformEventHandler<DeleteCloudFormationRequest> {
    ...
    public void accept(Event<DeleteCloudFormationRequest> event) {
        ...
        try {
            ...
            Optional<CloudResource> cfCloudResource = resourceRetriever
                    .findFirstByStatusAndTypeAndStack(CommonStatus.CREATED, ResourceType.CLOUDFORMATION_STACK, event.getData().getResourceId());
            boolean cloudFormationTemplateDeleted = false;
            if (cfCloudResource.isPresent()) {
                if (allInstancesDeletedFromCloudFormation(ac, cfCloudResource.get())) {
                    List<CloudResourceStatus> terminateResult = awsTerminateService.terminate(ac, cloudStack, List.of(cfCloudResource.get()));
                    ResourcePersisted resourceDeletionResult = resourceNotifier.notifyDeletion(cfCloudResource.get(), cloudContext);
                    ...
                    LOGGER.info("Cannot delete the Cloudformation because it contain instances");
                }
            } else {
    ...
    private boolean allInstancesDeletedFromCloudFormation(AuthenticatedContext ac, CloudResource cloudResource) {
